...
{
    if (is_child_theme()) {
        return get_stylesheet_directory() . fw_get_framework_customizations_dir_rel_path(
            $rel_path
        );
    } else {
...
{
    if (is_child_theme()) {
        return get_stylesheet_directory_uri() . fw_get_framework_customizations_dir_rel_path(
            $rel_path
        );
    } else {
        // check is_child_theme() before using this function
    ...
    try {
        ...
        FW_Cache::set(
            ...
            $dir = apply_filters(
                ...
                ) // double dirname() to remove '/helpers', use parent dir
            )
        );
    }
...
function fw_akg($keys, $array_or_object, $default_value = null, $keys_delimiter = '/')
{
    ...
    if ($is_object) {
        ...
        if (
            ! is_array($array_or_object) || ! array_key_exists(
                $key_or_property,
                $array_or_object
            )
        ) {
...
function fw_aks($keys, $value, &$array_or_object, $keys_delimiter = '/')
{
    ...
    if ($is_object) {
        if (
            ! property_exists($array_or_object, $key_or_property) || ! (is_array(
            ...
            if ($key_or_property === '') {
                ...
                trigger_error(
                    'Cannot push value to object like in array ($arr[] = $val)',
                    E_USER_WARNING
                );
            } else {
                $array_or_object->{$key_or_property} = [];
 ...
 */
function fw_html_tag($tag, $attr = [], $end = false)
{
    ...
    if (is_array($value)) {
        if ($magic_quotes) {
            ...
            foreach ($value as $key => $val) {
                $new_value[is_string($key) ? stripslashes($key) : $key] = fw_stripslashes_deep_keys(
                    $val
                );
            }
            $value = $new_value;
    ...
    if (is_array($value)) {
        if ($magic_quotes) {
            ...
            foreach ($value as $key => $value) {
                $new_value[is_string($key) ? addslashes($key) : $key] = fw_addslashes_deep_keys(
                    $value
                );
            }
            $value = $new_value;
...
function fw_current_screen_match(array $rules)
{
    ...
    $rules = array_merge(
        [
            'exclude' => [], // array of arrays or array with keys from $available_options
            'only'    => [], // same as in 'exclude'
        ],
        $rules
    );
...
if (! function_exists('fw_render_view')) :
     ...
     * @return string HTML
     */
    function fw_render_view($file_path, $view_variables = [], $return = true)
    {
...
function fw_collect_first_level_options(&$collected, &$options)
{
    ...
    if (empty($collected)) {
        $collected['tabs'] = $collected['boxes'] = $collected['groups'] = $collected['options'] = $collected['groups_and_options'] = $collected['all'] = [];
    }
    ...
    foreach ($options as $option_id => &$option) {
        if (isset($option['options'])) {
            ...
            switch ($option['type']) {
                ...
                default:
                    trigger_error(
                        'Invalid option container type: ' . $option['type'],
                        E_USER_WARNING
                    );
                    continue 2;
            }

            $collected['all'][$option['type'] . ':~:' . $option_id] = [
                'type'   => $option['type'],
                'id'     => $option_id,
                'option' => &$option,
            ];
        } elseif (
            is_int($option_id) && is_array($option) && /**
            ...
            $collected['all']['option' . ':~:' . $option_id] = [
                'type'   => 'option',
                'id'     => $option_id,
                'option' => &$option,
            ];
 ...
 */
function fw_collect_options(&$result, &$options, $settings = [], $_recursion_data = [])
{
    ...
    if (empty($_recursion_data)) {
        ...
        ! (isset($_recursion_data['access_key']) && ($_recursion_data['access_key'] instanceof FW_Access_Key) && ($_recursion_data['access_key']->get_key(
        ) === 'fw_collect_options'))
    ) {
    ...
    foreach ($options as $option_id => &$option) {
        if (isset($option['options'])) { // this is a container
            ...
            fw_collect_options(
                ...
                $option['options'],
                $settings,
                array_merge($_recursion_data, ['level' => $_recursion_data['level'] + 1])
            );
        } elseif (
            is_int($option_id) && is_array($option) && /**
             * make sure the array key was generated automatically
            ...
            if (
                is_array($settings['limit_option_types']) && ! in_array(
                    $option['type'],
                    $settings['limit_option_types']
                )
            ) {
                continue;
            }
...
{
    return ! (wp_is_post_revision($post_id) || wp_is_post_autosave($post_id) || (defined(
        'DOING_AUTOSAVE'
    ) && DOING_AUTOSAVE) || (defined(
        'DOING_AJAX'
    ) && DOING_AJAX) || empty($_POST) || empty($_POST['post_ID']) || $_POST['post_ID'] != $post_id);
}
    ...
    try {
        ...
        foreach ($g_fonts['items'] as $font) {
            $fonts[$font['family']] = [
                ...
                'position' => isset($old_fonts[$font['family']]) ? $old_fonts[$font['family']]['position'] : 99999,
            ];
        }
...
function fw_get_google_fonts_v2()
{
    ...
    if (false === $saved_data || ($saved_data['last_update'] + $ttl < time())) {
        ...
        if (
            200 === wp_remote_retrieve_response_code($response) && ! is_wp_error(
                $body
            ) && ! empty($body)
        ) {
            update_option(
...
function fw_is_post_edit()
{
    ...
    if ($result === null) {
        ...
        if (is_admin()) {
            if (empty($_POST) && isset($_GET['action']) && $_GET['action'] === 'edit' && isset($_GET['post'])) {
                // Display Edit Post page
                $result = true;
            } elseif (
                isset($_POST['action']) && $_POST['action'] === 'editpost' && isset($_POST['post_type']) && isset($_POST['post_ID']) && strpos(
                    wp_get_referer(
                    ),
                    'action=edit'
                ) !== false
 ...
 * @return bool|string
 */
function fw_oembed_get($url, $args = [])
{
    $html = wp_oembed_get($url, $args);

    if (
        ! empty($args['width']) and ! empty($args['height']) and class_exists(
            'DOMDocument'
        ) and ! empty($html)
    ) {
...
function fw_id_to_title($id)
{
    // mb_ucfirst()
    if (
        function_exists('mb_strtoupper') && function_exists('mb_substr') && function_exists(
            'mb_strlen'
        )
    ) {
 ...
 * @return string
 */
function fw_string_to_icon_html($icon, array $attributes = [])
{
    if (
        preg_match('/\.(png|jpg|jpeg|gif|svg|webp)$/', $icon) || preg_match(
            '/^data:image\//',
            $icon
        )
    ) {
 ...
 * @return array
 */
function fw_get_mime_type_by_ext($type = [])
{
    $result = [];
 ...
 *
 */
function fw_multi_ext2type($ext_array = [])
{
    $result = [];
 ...
 * @since 2.6.14
 */
function fw_callback($callback, array $args = [], $cache = true)
{
...
function fw_call($value)
{
    ...
    return (is_object($value) && get_class($value) == 'Closure') ? $value() : $value->execute();
}
