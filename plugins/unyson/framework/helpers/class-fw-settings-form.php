...
abstract class FW_Settings_Form
{
    ...
    final public function __construct($id)
    {
        ...
        $this->id = $id;
        $this->fw_form = new FW_Form(
            'fw-settings-form:' . $this->get_id(),
            [
            'render' => [$this, '_form_render'],
            'validate' => [$this, '_form_validate'],
            'save' => [$this, '_form_save'],
            ]
        );
        $this->strings = [
    ...
    final public function render()
    {
...

        {
            remove_action( // In case render is called multiple times
                ...
                [$this, '_action_admin_print_footer_scripts']
            );
            add_action(
                'admin_print_footer_scripts',
                [$this, '_action_admin_print_footer_scripts'],
    ...
    public function _form_render($data)
    {
        ...
        fw_render_view(
            fw_get_framework_directory('/views/backend-settings-form.php'),
            [
                'form' => $this,
                'values' => (
    ...
    public function _form_save($data)
    {
        ...
        if (!empty($_POST[self::$input_name_reset])) { // The "Reset" button was pressed
            ...
            $new_values = $this->is_theme_settings()
                ? apply_filters('fw_settings_form_reset:values', [], $old_values)
                : apply_filters('fw:settings-form:' . $this->get_id() . ':reset:values', [], $old_values);
