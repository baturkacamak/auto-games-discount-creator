...
class MarketLocalizedRoundupsSitemapProvider extends WP_Sitemaps_Provider
{
	...
	public function get_url_list($page_num, $object_subtype = ''): array
	{
		...
		foreach ($query->posts as $post) {
			...
			$location = $this->buildMarketPermalink($post, $marketKey);
			if ($location === '') {
				continue;
			}
	...
	private function buildMarketPermalink(WP_Post $post, string $marketKey): string
	{
		...
		return $baseUrl . '/' . trim($marketKey, '/') . '/' . $slug . '/';
	}
}
